...
class mapOptimization : public ParamServer
{
   public:
    ...
    mapOptimization(const rclcpp::NodeOptions& options) : ParamServer("liorf_localization_mapOptimization", options)
    {
        ...
        //                 std::bind(&mapOptimization::saveMapService, this, std::placeholders::_1,
        //                 std::placeholders::_2 ));
    ...
    void loadGlobalMap()
    {
        ...
        if (laserCloudSurfFromMapDSNum < 1000) return;
    ...
    {
        if (!has_global_map) return false;
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++)
        {
            ...
            float ary =
                (-crx * sry * pointOri.x + crx * cry * srz * pointOri.y + crx * cry * crz * pointOri.z) * coeff.x +
