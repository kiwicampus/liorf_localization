...
mapOptimization::mapOptimization(const rclcpp::NodeOptions& options)
    : ParamServer("liorf_localization_mapOptimization", options)
{
    ...
    pubLaserOdometryGlobal = create_publisher<nav_msgs::msg::Odometry>("liorf_localization/mapping/odometry",
                                                                       QosPolicy(history_policy, reliability_policy));
    ...
    pubIcpKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>(
        "liorf_localization/mapping/icp_loop_closure_corrected_cloud", QosPolicy(history_policy, reliability_policy));
    pubRecentKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("liorf_localization/mapping/map_local",
                                                                         QosPolicy(history_policy, reliability_policy));
    pubRecentKeyFrame = create_publisher<sensor_msgs::msg::PointCloud2>("liorf_localization/mapping/cloud_registered",
                                                                        QosPolicy(history_policy, reliability_policy));
    ...
    pubSLAMInfo = create_publisher<liorf_localization::msg::CloudInfo>("liorf_localization/mapping/slam_info",
                                                                       QosPolicy(history_policy, reliability_policy));
    ...
    pubLocalizationInfo = create_publisher<liorf_localization::msg::LocalizationInfo>(
        "liorf_localization/mapping/localization_info", QosPolicy(history_policy, reliability_policy));
    pubLocalizationRestored = create_publisher<std_msgs::msg::Empty>("liorf_localization/mapping/localization_restored",
                                                                     QosPolicy(history_policy, reliability_policy));
    ...
    pubGlobalMap = create_publisher<sensor_msgs::msg::PointCloud2>("liorf_localization/localization/global_map",
                                                                   rclcpp::QoS(10).reliable(), pub_options);

    br = std::make_unique<tf2_ros::TransformBroadcaster>(this);
    // Initialize the TF buffer and listener
    tf_buffer_ =
        std::make_unique<tf2_ros::Buffer>(this->get_clock(), tf2::Duration(tf2::BUFFER_CORE_DEFAULT_CACHE_TIME),
                                          std::shared_ptr<rclcpp::Node>(this, [](rclcpp::Node*) {}));
    tf_listener_ = std::make_unique<tf2_ros::TransformListener>(
        *tf_buffer_, std::shared_ptr<rclcpp::Node>(this, [](rclcpp::Node*) {}),
        false);  // Pass false to not use dynamic transform topic
    ...
    downSizeFilterLocalMapSurf.setLeafSize(surroundingKeyframeMapLeafSize, surroundingKeyframeMapLeafSize,
    ...
    initial_guess_max_iters_ =
        std::getenv("INITIAL_GUESS_MAX_ITERS") ? std::stoi(std::getenv("INITIAL_GUESS_MAX_ITERS")) : 0;
    ...
    params_callback_handle_ = this->add_on_set_parameters_callback(
        std::bind(&mapOptimization::on_parameters_set_callback, this, std::placeholders::_1));
...
rcl_interfaces::msg::SetParametersResult mapOptimization::on_parameters_set_callback(
    const std::vector<rclcpp::Parameter>& parameters)
{
    ...
    for (const auto& param : parameters)
    {
        if (param.get_name() == "orientation_change_threshold")
        {
            ...
            RCLCPP_INFO(this->get_logger(), "orientation_change_threshold updated to %f",
                        orientation_change_threshold_);
        }
...
void mapOptimization::processReceivedMap(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    // Check if this is the same map we already have (deduplication)
    size_t current_size = msg->width * msg->height;
    size_t current_hash =
        std::hash<std::string>{}(std::string(reinterpret_cast<const char*>(msg->data.data()),
...
void mapOptimization::processLoadedMap(pcl::PointCloud<PointType>::Ptr laserCloudSurfFromMap)
{
...
void mapOptimization::initialposeHandler(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msgIn)
{
    ...
    try
    {
        ...
        base_link_to_livox_.translate(Eigen::Vector3f(transformStamped.transform.translation.x,
    ...
    tf2::Quaternion q(msgIn->pose.pose.orientation.x, msgIn->pose.pose.orientation.y, msgIn->pose.pose.orientation.z,
...
void mapOptimization::laserCloudInfoHandler(const liorf_localization::msg::CloudInfo::SharedPtr msgIn)
{
    // extract time stamp
...
bool mapOptimization::systemInitialize()
{
    ...
    std::cout << "average x in transformed point cloud: " << out_cloud->points[out_cloud->size() / 2].x
              << " and average x in original point cloud: "
              << laserCloudSurfLast->points[laserCloudSurfLast->size() / 2].x << std::endl;
    ...
    RCLCPP_INFO(get_logger(), "Cropped map from %d to %d points (radius: %.1f m)", laserCloudSurfFromMapDS->size(),
                cropped_map->size(), crop_radius);
    ...
    *cloudOut += *transformPointCloud(laserCloudSurfLast, &thisPose6D);
    ...
    if (icp.hasConverged() && icp.getFitnessScore() < icp_fitness_score_threshold_)
    {
        ...
        RCLCPP_INFO(get_logger(),
                    "initializing pose accepted, original pose: %f, %f, yaw: %f, refined pose: %f, %f, yaw: %f. used "
                    "pointcloud with %d points",
...
void mapOptimization::gpsHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
{
...
bool mapOptimization::saveMapService(const std::shared_ptr<liorf_localization::srv::SaveMap::Request> req,
...
void mapOptimization::visualizeGlobalMapThread()
{
    ...
    std::shared_ptr<liorf_localization::srv::SaveMap::Request> req =
        std::make_unique<liorf_localization::srv::SaveMap::Request>();
    std::shared_ptr<liorf_localization::srv::SaveMap::Response> res =
        std::make_unique<liorf_localization::srv::SaveMap::Response>();
...
void mapOptimization::adjustForRotation()
{
...
void mapOptimization::updatePointAssociateToMap() { transPointAssociateToMap = trans2Affine3f(transformTobeMapped); }
...
bool mapOptimization::LMOptimization(int iterCount)
{
    ...
    for (int i = 0; i < laserCloudSelNum; i++)
    {
        ...
        float arz =
            ((crx * sry * crz + srx * srz) * pointOri.y + (srx * crz - crx * sry * srz) * pointOri.z) * coeff.x +
    ...
    float deltaR = sqrt(pow(pcl::rad2deg(matX(0)), 2) + pow(pcl::rad2deg(matX(1)), 2) + pow(pcl::rad2deg(matX(2)), 2));
    float deltaT = sqrt(pow(matX(3) * 100, 2) + pow(matX(4) * 100, 2) + pow(matX(5) * 100, 2));
...
void mapOptimization::updateMatchQualityMetrics(bool optimization_converged)
{
...
void mapOptimization::updateMapFitnessMetrics()
{
    ...
    const size_t p90_idx = static_cast<size_t>(0.9f * static_cast<float>(distances.size() - 1));
...
bool mapOptimization::scan2MapOptimization()
{
...
void mapOptimization::addGPSFactor()
{
    ...
    {
        if (common_lib_->pointDistance(cloudKeyPoses3D->front(), cloudKeyPoses3D->back()) < 5.0) return;
    }
        ...
        else
        {
            ...
            if (distance > mappingGpsDistanceThreshold)
            {
                RCLCPP_WARN_THROTTLE(
                    ...
                    "GPS measurement rejected: too far from current estimate (%.2f m > %.2f m threshold)", distance,
                    mappingGpsDistanceThreshold);
                continue;
            }
...
void mapOptimization::publishOdometry()
{
    ...
    msgLocalizationInfo.map_pose.covariance =
        matrixToArray(poseCovariance, std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
    ...
    // time_point, mapFrame); geometry_msgs::msg::TransformStamped trans_odom_to_lidar; tf2::convert(temp_odom_to_lidar,
    // trans_odom_to_lidar); trans_odom_to_lidar.child_frame_id = lidarFrame; br->sendTransform(trans_odom_to_lidar);
...
void mapOptimization::setInitialPoseCallback(
